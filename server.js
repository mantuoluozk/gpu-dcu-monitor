...
function buildSystemCommand(command) {
  ...
  return [
    ...
    "cpu_power_source=''; cpu_power_scope='';",
    "cpu_power=$(awk -v b=\"$energy_before\" -v a=\"$energy_after\" 'BEGIN{if(a>b && b>0) printf \"%.1f\", (a-b)/1000000/0.2}');",
    "if [ -n \"$cpu_power\" ]; then cpu_power_source='powercap'; cpu_power_scope='package'; fi;",
    "if [ -z \"$cpu_power\" ]; then cpu_power=$(awk '{sum+=$1} END{if(sum>0) printf \"%.1f\", sum/1000000}' /sys/class/hwmon/hwmon*/power*_input 2>/dev/null); if [ -n \"$cpu_power\" ]; then cpu_power_source='hwmon'; cpu_power_scope='host'; fi; fi;",
    "if [ -z \"$cpu_power\" ]; then cpu_power=$(sensors 2>/dev/null | awk '/CPU|Package|PPT|Power/ {for(i=1;i<=NF;i++) if($i ~ /^[0-9.]+W$/){gsub(/[^0-9.]/,\"\",$i); sum+=$i}} END{if(sum>0) printf \"%.1f\", sum}'); if [ -n \"$cpu_power\" ]; then cpu_power_source='sensors'; cpu_power_scope='sensor'; fi; fi;",
    ...
    "cpu_model=$(awk '/model name|Hardware|Processor/ {line=$0; sub(/^[^:]*:[ \\t]*/, \"\", line); if(line){print line; exit}}' /proc/cpuinfo 2>/dev/null);",
    "cpu_lscpu_model=$(lscpu 2>/dev/null | awk '/Model name:/ {line=$0; sub(/^[^:]*:[ \\t]*/, \"\", line); print line; exit}');",
    "cpu_dmi_models=$(dmidecode -t processor 2>/dev/null | awk 'function trim(v){gsub(/^[ \\t]+|[ \\t]+$/, \"\", v); return v} function value(v){sub(/^[^:]*:[ \\t]*/, \"\", v); return trim(v)} function useful(v){return v && v !~ /Not Specified|To Be Filled|Unknown|None/i} function commit(){v=useful(version)?version:part; if(useful(v)){count[v]++} version=\"\"; part=\"\"} /^Handle /{commit()} /^[ \\t]*Version:/ {version=value($0)} /^[ \\t]*Part Number:/ {part=value($0)} END{commit(); first=1; for(v in count){if(!first) printf \"; \"; printf \"%s x%s\", v, count[v]; first=0}}');",
    "dmidecode -t processor 2>/dev/null | awk 'function trim(v){gsub(/^[ \\t]+|[ \\t]+$/, \"\", v); return v} function value(v){sub(/^[^:]*:[ \\t]*/, \"\", v); return trim(v)} function clean(v){return v ~ /Not Specified|To Be Filled|Unknown|None/i ? \"\" : v} function emit(){version=clean(version); part=clean(part); if(version || part || socket){printf \"SYSCPU\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\n\", socket, version, part, cores, threads, maxspeed, curspeed} socket=\"\"; version=\"\"; part=\"\"; cores=\"\"; threads=\"\"; maxspeed=\"\"; curspeed=\"\"} /^Handle /{emit()} /^[ \\t]*Socket Designation:/ {socket=value($0)} /^[ \\t]*Version:/ {version=value($0)} /^[ \\t]*Part Number:/ {part=value($0)} /^[ \\t]*Core Count:/ {cores=value($0)} /^[ \\t]*Thread Count:/ {threads=value($0)} /^[ \\t]*Max Speed:/ {maxspeed=value($0)} /^[ \\t]*Current Speed:/ {curspeed=value($0)} END{emit()}';",
    "cpu_sockets=$(lscpu 2>/dev/null | awk -F: '/^Socket\\(s\\):/ {gsub(/^[ \\t]+/, \"\", $2); print $2; exit}');",
    "cpu_models=$(awk 'function trim(v){gsub(/^[ \\t]+|[ \\t]+$/, \"\", v); return v} function value(v){sub(/^[^:]*:[ \\t]*/, \"\", v); return trim(v)} function commit(){if(model){if(phys!=\"\"){hasphys=1; key=phys SUBSEP model; socketModel[key]=model}else{fallback[model]=1}} model=\"\"; phys=\"\"} /^processor[ \\t]*:/ {commit(); next} /model name|Hardware|Processor/ {if(!model) model=value($0)} /physical id/ {phys=value($0)} END{commit(); if(hasphys){for(k in socketModel){count[socketModel[k]]++; sockets++}}else{for(m in fallback){count[m]++; sockets++}} first=1; for(m in count){if(!first) printf \"; \"; printf \"%s x%s\", m, count[m]; first=0} printf \"\\nSOCKETS\\t%s\", sockets}' /proc/cpuinfo 2>/dev/null);",
    ...
    "printf 'SYS\\tcpuTemperatureSource\\t%s\\n' \"$cpu_temp_source\";",
    "printf 'SYS\\tcpuPowerW\\t%s\\n' \"$cpu_power\";",
    "printf 'SYS\\tcpuPowerSource\\t%s\\n' \"$cpu_power_source\";",
    "printf 'SYS\\tcpuPowerScope\\t%s\\n' \"$cpu_power_scope\";",
