...
- 首页服务器卡片视觉基准来自用户提供的 `C:\Users\zhengke\Desktop\index.html`：标题栏与状态徽标独立、占用主视觉使用状态渐变浅底色、快速指标为三等分、硬件信息为两列、GPU/DCU 设备为 2 列 × 4 行、资产与心跳位于底栏。颜色、字号、16px 圆角、轻边框、图标和悬浮阴影应严格沿用该 demo，不要为了压缩高度自行改变设备网格。
