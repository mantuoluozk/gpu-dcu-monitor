...
项目版本变化和部署兼容信息请查看：[完整更新日志](CHANGELOG.md)。
...
- 按分钟记录整机 CPU 使用率及 GPU/DCU 算力、显存、温度和功耗历史，支持详情页趋势查看和 CSV 导出。
- CPU 型号会优先读取 `dmidecode` 中的具体处理器版本/部件信息，DCU 卡详情会尽量展示 CU 数；目标机器不暴露对应信息时显示为空。
- CPU 板块支持按物理 CPU 翻页查看 Socket、具体型号、部件号、核心/线程和频率；系统负载的三个数字分别是 1 分钟、5 分钟、15 分钟 load average。
- 服务器详情页按 CPU、内存、系统版本、DCU/GPU 卡状态、模型与镜像分块展示，并支持只刷新当前服务器。
- 首页服务器卡片会显示 CPU 型号、系统版本和驱动版本摘要，便于快速区分不同硬件/软件环境。
- 缺少 GPU/DCU 型号或 DCU CU 缓存的机器会在状态轮询时自动补采一次拓扑信息，BW100/BW150/BW1000 等明确型号在目标机缺少 `rocminfo` 时也会按型号兜底显示 CU；补齐后后续刷新仍保持轻量采集。
...
- CPU 与 GPU/DCU 历史数据保存在 `data/history/`，不会进入 Git 或普通代码部署包；迁移部署服务器时需要单独备份和恢复该目录。
